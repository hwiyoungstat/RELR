...
Multi_ELR <- function(grid,X,Response,H){
  ...
  W <- sapply(1:N.eval, function(i) dmvnorm((X-matrix(rep(grid[i,],each=N),ncol=m)/h))/h,simplify="matrix") # Calculate Weight (Row = CV for model, COl=VAlidataion(evaluation point))
  W.Sum <- colSums(W)
